...
def get_json_metadata():
    ...
    log_message(f"Metadata loaded: {json_metadata}")  # Added for debugging
...
def delete_app():
    ...
    try:
        ...
        if error_code == 'AccessDeniedException' or error_code == 'NotAuthorized':
            ...
            log_message(f"[auto-stop-idle] - An error occurred while deleting app. Exception: {client_error}")
    except Exception as e:
        log_message(f"[auto-stop-idle] - An error occurred while deleting app. Exception: {e}")
...
def create_state_file(state_file_path):
    ...
    if not os.path.exists(state_file_path):
        with open(state_file_path, 'w') as f:
            ...
            log_message(f"State file created with date: {current_date_as_string}")  # Added for debugging
...
def get_state_file_contents(state_file_path):
    ...
    with open(state_file_path) as f:
        contents = f.readline()
    log_message(f"State file contents: {contents}")  # Added for debugging
    return contents
...
def update_state_file(sessions, terminals, state_file_path):
    ...
    if sessions is not None and len(sessions) > 0:
        for notebook_session in sessions:
            ...
            if last_activity_date > max_last_activity:
                ...
                log_message(f"Session {notebook_session['id']} last_activity updated to: {last_activity}")  # Added for debugging
    if terminals is not None and len(terminals) > 0:
        for terminal in terminals:
            ...
            if last_activity_date > max_last_activity:
                max_last_activity = last_activity_date
                log_message(f"Terminal {terminal['name']} last_activity updated to: {last_activity}")  # Added for debugging
...
def is_idle(idle_time, last_activity):
    ...
    """
    last_activity_date = datetime.strptime(last_activity, DATE_FORMAT)
    idle_check = (datetime.now() - last_activity_date).total_seconds() > idle_time
    log_message(f"Checking if idle based on last activity: {last_activity_date}, Idle: {idle_check}")  # Added for debugging
    return idle_check
...
def get_terminals(app_url, base_url):
    ...
    log_message(f"Obtained Terminals: {terminals}")  # Added for debugging
    return terminals
...
def get_sessions(app_url, base_url):
    ...
    log_message(f"Obtained Sessions: {sessions}")  # Added for debugging
    return sessions
...
def check_idle(app_url, base_url, idle_time, ignore_connections, skip_terminals, state_file_path):
    ...
    log_message(f"Terminal count: {terminal_count}, Session count: {session_count}")  # Added for debugging
...
# Main script continues with argument parsing and the rest of the logic...
